  ...
  const content = `<section class="home-hero">
    <div class="shell hero-grid">
      ...
      <div class="hero-system" aria-label="${fr ? "Applications Studio501 pour Windows et Android" : "Studio501 applications for Windows and Android"}" data-reveal>
        ...
        <div class="floating-app floating-app--three">${iconMarkup(appBySlug("myhomeassistant"), "floating-icon")}<span>MyHomeAssistant</span></div>
        <div class="platform-chip platform-chip--windows">Windows</div><div class="platform-chip platform-chip--android">Android</div>
      </div>
    </div>
  </section>
  <section class="section"><div class="shell"><div class="section-heading"><div><p class="eyebrow">${fr ? "À découvrir" : "Featured"}</p><h2>${fr ? "Applications mises en avant" : "Featured applications"}</h2></div><a class="text-link" href="${langPath(lang, "/apps/")}">${fr ? "Voir les" : "View all"} ${apps.length} applications<span aria-hidden="true"> →</span></a></div><div class="app-grid">${featured.map((app) => appCard(app, lang)).join("")}</div></div></section>
...
function platformPage(lang, platformKey) {
  ...
  const intro = isWindows ? {
    ...
    kicker: "Studio501 · Google Play", heading: fr ? "Applications Android" : "Android applications", lead: fr ? `${publishedPlatformCount} applications premium sont publiées sur Google Play, toujours sans publicité ni compte obligatoire.` : `${publishedPlatformCount} premium applications are published on Google Play, still with no advertising or mandatory account.`,
  };
...
function applicationPage(lang, app) {
  ...
  <section class="section"><div class="shell"><div class="section-heading"><div><p class="eyebrow">${t.features}</p><h2>${esc(app.featuresTitle?.[lang] || (fr ? "Tout ce qu’il faut, clairement." : "Everything you need, clearly."))}</h2></div></div><ul class="feature-grid">${app.features[lang].map((feature) => `<li data-reveal><span aria-hidden="true">✓</span>${esc(feature)}</li>`).join("")}</ul></div></section>
